...
public class ScoreboardManager {
    ...
    void setupScoreboard(MinecraftServer server) {
        ...
        // Use getNullableObjective instead of containsObjective (containsObjective is missing in
        // 1.16.1 intermediary)
    ...
    void update(
        ...
        new ArrayList<>(scores)
                .forEach(score -> scoreboard.resetPlayerScore(score.getPlayerName(), objective));

        // Show scoreboard entries only when the world goal is met (in the trigger player's
        // language)
        if (finished) {
