...
public class SwapMod implements ModInitializer {
    ...
    /**
     * Players set to locked-spectator mode via "become_spectator" from Velocity.
     * Their camera is periodically re-locked to the active survival player.
     * Maps watcher UUID → UUID of the player they are locked to.
     */
    private final Map<UUID, UUID> spectatorCameras = new HashMap<>();
    /**
     * Players currently in a dead state (health ≤ 0).
     * Used to detect the exact tick of death so we can clear the saved inventory
     * and prevent item duplication on swap (items remain on the floor in the world;
     * the next player starts with an empty inventory and can pick them up).
     */
    ...
    /**
     * Deferred state restore: ENTITY_LOAD schedules the restore for X ticks later.
     * This gives the client connection time to stabilise and prevents an interaction freeze.
     */
    ...
    public void onInitialize() {
            ...
            if (!connectedPlayers.contains(player.getUuid())) {
                ...
                if (daylightCycleFrozen) {
                    ...
                    server.getWorlds().forEach(w ->
                            w.getGameRules().get(GameRules.DO_DAYLIGHT_CYCLE).set(true, server));
                }
                scoreboardManager.update(finished, completedWorlds, requiredWorlds, currentTime, player);
                PlayerState cs = stateManager.getCurrentState();
                if (cs != null) {
                    // Capture joining player's own hotbar preference NOW (their own server NBT)
                    // before restoreState overwrites the inventory with the predecessor's items.
                    if (stateManager.saveHotbar) {
                        net.minecraft.item.Item[] pref = new net.minecraft.item.Item[9];
                        for (int i = 0; i < 9; i++) pref[i] = player.inventory.getStack(i).getItem();
                        stateManager.hotbarPreferences.put(player.getUuid(), pref);
                    }
                    // Pre-set position immediately so the client never renders the old spawn
                    // location. Only applicable when staying in the same dimension; cross-
                    // dimension joins go through a loading screen so there's no visible flash.
        ...
        ServerTickEvents.END_SERVER_TICK.register(srv -> {
            ...
            for (Iterator<Map.Entry<UUID, Integer>> it = restoreCountdown.entrySet().iterator(); it.hasNext(); ) {
            ...
            if (stateSaveTick >= 20) {
                ...
                for (ServerPlayerEntity p : srv.getPlayerManager().getPlayerList()) {
                    ...
                    // Only save living survival players (dead players keep the last living save).
                ...
                for (Map.Entry<UUID, UUID> cam : spectatorCameras.entrySet()) {
                    ServerPlayerEntity watcher = srv.getPlayerManager().getPlayer(cam.getKey());
                    ServerPlayerEntity target  = srv.getPlayerManager().getPlayer(cam.getValue());
    ...
    private void onPlayerExitEnd(ServerPlayerEntity player) {
        ...
        server.getPlayerManager().getPlayerList().forEach(p ->
                p.sendMessage(new LiteralText(Lang.gameFinished(p.getUuid())), false)
        );
        ...
        switch (sub) {
            ...
            case "save":
                ...
                for (ServerPlayerEntity p : server.getPlayerManager().getPlayerList()) {
                    if (p.interactionManager.getGameMode() != GameMode.SPECTATOR && p.getHealth() > 0.0f) {
                        stateManager.saveState(p);
                    }
                }
                return;
            ...
            case "prepare_return": {
                ...
                target.teleport(target.getServerWorld(),
                        target.getX(), 100000, target.getZ(), target.yaw, target.pitch);
    ...
    private void resetWorldState() {
        ...
        server.getPlayerManager().getPlayerList().forEach(p ->
                p.sendMessage(new LiteralText(Lang.newRound(p.getUuid())), false)
        );
