...
These must be present in `mods/` alongside `mcsrswap-fabric-mod-1.0.21.jar`:
...
- Carpet - used by MCSR-Swap to freeze tickspeed before players join server and during swaps
...
Copy `mcsrswap-velocity-plugin-1.0.21.jar` into `plugins/`.
...
saveHotbar: true           # On player swap, rearrange the server's hotbar items to match the joining player's preferred layout
...
|---|---|---|
| `rotationTime` | `120` | Seconds between rotations. |
| `requiredPercentage` | `1.0` | Fraction of worlds that must be beaten to win (`1.0` = all, `0.5` = half). |
| `versus` | `false` | Two-team versus mode. |
| `language` | `en_us.yml` | Language file from `plugins/mcsrswap/languages/`. |
| `gameServerPrefix` | `game` | Velocity server names with this prefix are treated as game servers. |
| `lobbyServerName` | `lobby` | Exact Velocity server name for the lobby. |
| `spectateAfterWin` | `false` | Send world-beaten players to spectate an adjacent server until next rotation. |
| `spectateTarget` | `next` | Which adjacent server to spectate: `next` or `prev`. |
| `spectateMinTime` | `15` | Minimum seconds remaining in the rotation to activate spectate. |
| `saveHotbar` | `true` | Rearrange the server's hotbar to match the joining player's preferred layout on each swap. |
| `eyeHoverTicks` | `80` | Eye of Ender hover lifetime in ticks. Increase for easier NinjabrainBot readings on multiplayer (no pause-buffering). |
| `admins` | – | List of player UUIDs or usernames that bypass permission checks. |
| `ignorePlayers` | – | List of player UUIDs or usernames excluded from game starts by default (toggleable per session with `/ms ignore`). |
...
```
mcsrswap-fabric-mod-1.0.21.jar
