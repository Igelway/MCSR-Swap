...
|---|---|
| `/ms start` says "Game is already running" | Stop the current game first with `/ms stop` |
| Players are not rotated | Server names must start with `game` in `velocity.toml` |
| Eye of Ender falls too fast | Increase `eyeHoverTicks` in `config/mcsrswap/config.properties` on each game server |
